...
class StorageService:
    ...
    async def delete(self, storage_path: str) -> None:
        if settings.STORAGE_BACKEND == "cloudinary":
            await self._delete_cloudinary(storage_path)
    ...
    async def _delete_cloudinary(self, storage_path: str) -> None:
        import asyncio
        import re

        # Extract public_id from the full Cloudinary URL.
        # URL format: https://res.cloudinary.com/{cloud}/image/upload/v{ver}/{public_id}.{ext}
        match = re.search(r"/upload/(?:v\d+/)?(.+?)(?:\.[^./]+)?$", storage_path)
        if not match:
            logger.warning("Cannot extract Cloudinary public_id", url=storage_path)
            return
        ...
        loop = asyncio.get_running_loop()
        try:
            await loop.run_in_executor(None, lambda: uploader.destroy(public_id))
            logger.info("Deleted Cloudinary image", public_id=public_id)
        except Exception as exc:
            raise StorageError(f"Cloudinary deletion failed: {exc}") from exc
