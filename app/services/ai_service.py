...
class LocalONNXEngine(InferenceEngine):
    ...
    def _run_sync(
        ...
        # If the model finds zero objects of any class the image is very likely
        # not a blood smear (a real slide always has visible cells).
        if not detections:
            from app.core.exceptions import ImageValidationError
            raise ImageValidationError(
                "No blood cells detected in this image. "
                "Please upload a Giemsa-stained blood smear microscopy slide."
            )
...
class LocalYOLOv9Engine(InferenceEngine):
    ...
    def _run_sync(
        ...
        # If the model finds zero objects the image is very likely not a blood smear.
        total_detections = sum(
            len(res.boxes) for res in yolo_results if res.boxes is not None
        )
