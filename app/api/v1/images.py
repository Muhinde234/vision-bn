...
async def upload_image(
    ...
    try:
        ...
        run_inference_task.delay(str(image_record.id), str(diagnosis_id))
        celery_queued = True
        logger.info("Inference queued via Celery", image_id=str(image_record.id))
    except Exception as exc:
        logger.warning("Celery unavailable, using background task", error=str(exc))
