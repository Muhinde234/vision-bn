...
async def forgot_password(
    ...
    """Request a password reset link. Sent via email if the address is registered."""
    service = AuthService(db)
    await service.create_reset_token(payload.email)
    return APIResponse(message="If that email is registered, you will receive a reset link shortly")
...
@router.get("/verify-email", response_model=APIResponse[UserRead])
async def verify_email(
    ...
    await db.commit()
    return APIResponse(message="Email verified successfully", data=UserRead.model_validate(user))
